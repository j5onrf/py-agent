...
<h1 align="center">Py Agent <img src="https://shieldcn.dev/badge/version-v0.9.9.11.svg?variant=secondary" alt="Version"><a href="https://github.com/j5onrf/py-agent"></a></h1>
...
Lightweight Python orchestration (`rich` + `requests` + `sqlite-vec` + `uvloop`) controlling a C++ backend `llama-server`. Optimized for fine-tuned quantized local models (`Qwen3.5-2B+` for chat, `Qwen3.8-27B` / `Qwen3.6-35B` for agents) and cloud providers—supporting native JSON tool calling, modular sub-27B adapters (`agent_adapters.py`), and a stateful NOOA IPython kernel REPL (`/py`).
...
- **llama.cpp WebAgent (`/webui`):** Autonomous tool-enabled web gateway on official `llama-server` UI (`:3000`) with auxiliary Gemini Flash Lite vision pre-processing.

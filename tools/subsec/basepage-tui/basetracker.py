...
TRACK_TARGETS = {
    "1": {
        "name": "Hugging Face: HauhauCS",
        "type": "huggingface_user",
        "id": "HauhauCS",
    },
    "2": {"name": "Hugging Face: Unsloth", "type": "huggingface_org", "id": "unsloth"},
    "3": {
        "name": "Hugging Face: mradermacher",
        "type": "huggingface_user",
        "id": "mradermacher",
    },
    "4": {
        "name": "Hugging Face: Bartowski",
        "type": "huggingface_user",
        "id": "bartowski",
    },
    "5": {
        "name": "Hugging Face: MaziyarPanahi",
        "type": "huggingface_user",
        "id": "MaziyarPanahi",
    },
    "6": {
        "name": "Hugging Face: Arcee AI",
        "type": "huggingface_org",
        "id": "arcee-ai",
    },
}
...
def print_header(subtitle=""):
    ...
    print(
        "         \033[90m│\033[0m   \033[1;32m󰚌  B A S E T R A C K E R\033[0m   \033[90m│\033[0m"
    )
    print("         \033[90m└──────────────────────────────┘\033[0m")
    if subtitle:
...
def render_target_view(target_config):
    ...
    if target_config["type"] in ("huggingface_user", "huggingface_org"):
        ...
        while True:
            ...
            header_text = (
                f"{target_name}  │  Type: {target_type}  │  Snapshots (Max {LIMIT})"
            )
            ...
            sys.stdout.write(
                " \033[1;35m󰚌  RECENTLY UPDATED / MODIFIED\033[0m\033[K\r\n"
            )
            ...
            if not modified_models:
                ...
                for idx, model in enumerate(modified_models[:LIMIT], 1):
                    ...
                    sys.stdout.write(
                        f"  {idx:2d}. \033[1;32m{display_name}\033[0m{padding_str}{badge_str}  {color}{date_str}\033[0m\033[K\r\n"
                    )
            ...
            sys.stdout.write(
                " \033[1;33m󰚌  NEWEST REPOSITORIES CREATED\033[0m\033[K\r\n"
            )
            sys.stdout.write(f"  \033[90m{'_' * (W - 6)}\033[0m\033[K\r\n\r\n")
            ...
            if not created_models:
                ...
                for idx, model in enumerate(created_models[:LIMIT], 1):
                    ...
                    color = "\033[0m" if idx % 2 == 0 else "\033[38;5;246m"
                    sys.stdout.write(
                        f"  {idx:2d}. \033[1;36m{display_name}\033[0m{padding_str}{badge_str}  {color}{date_str}\033[0m\033[K\r\n"
                    )
            ...
            sys.stdout.write(
                " \033[90m[q/Enter] Return to Target Selection\033[0m\033[K\r\n"
            )
            sys.stdout.flush()
...
def main():
    ...
    try:
        while True:
            ...
            options = [TRACK_TARGETS[k]["name"] for k in keys] + [
                "Exit Tracking Utility"
            ]

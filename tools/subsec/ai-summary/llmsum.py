...
def get_input_with_back_button(profile_name):
    ...
    try:
        while True:
            ...
            sys.stdout.write(
                "\033[90m[Copy your transcript, text, or PDF to clipboard before submitting]\033[0m\n\n"
            )
...
def run_menu():
    ...
    try:
        while True:
            ...
            tts_status = (
                "\033[1;32m[TTS: ON]\033[0m"
                if TTS_ENABLED
                else "\033[1;31m[TTS: OFF]\033[0m"
            )
            print(
                f" Welcome to the AI Summary TUI.           Status: {server_status}  {tts_status}"
            )
            print(
                " Use arrow keys to navigate, Enter to select, 't' to toggle speech\n"
            )
            for i, opt in enumerate(options):
...
def call_llm(user_input, system_prompt):
    ...
    try:
        ...
        if gemini_key:
            ...
            raise Exception(
                f"Connection to local backend failed. (Is your server online? {e.reason})"
            )
...
def render_and_read_summary(raw_text):
    ...
    try:
        ...
        subprocess.run(
            koko_cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True
        )
    ...
    while True:
        ...
        if user_input:
            ...
            try:
                ...
                if not gemini_key:
                    print(
                        "\033[1;33mEnsure your llama.cpp server is running. Press F8 to launch it!\033[0m"
                    )
            flush_input_buffer()

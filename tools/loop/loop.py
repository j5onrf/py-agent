#!/usr/bin/env python3
"""Autonomous Task Loop Engine [Hardened Production Ready]"""
...
TOOL_ERROR_PATTERN = re.compile(
    r"\[(?:tool )?error\]|\[denied\]|\[security\]|Traceback \(most recent call last\):|\bFAILED \((?:failures|errors)=\b",
    re.IGNORECASE,
)

MAX_SPEC_FILE_BYTES = 100 * 1024  # 100 KiB cap


def _read_spec_file(workspace: str, file_name: str = "TASK.md") -> str:
    target = os.path.realpath(os.path.join(workspace, file_name))
    if os.path.isfile(target):
        try:
            with open(target, "r", encoding="utf-8", errors="replace") as f:
                return f.read(MAX_SPEC_FILE_BYTES).strip()
        except OSError:
            pass
    return ""
...
def _get_thinking_budget(st: dict[str, Any]) -> int:
    env_budget = os.environ.get("AI_REASONING_BUDGET", "").strip()
    if env_budget:
        try:
            return max(0, int(env_budget))
        except ValueError:
            pass
    if st.get("reasoning_active", False):
        try:
            return max(0, int(st.get("reasoning_budget", 0)))
...
def run_task_loop(
    ...
    prev_confirm_gates = os.environ.get("AI_CONFIRM_GATES")
    os.environ["AI_CONFIRM_GATES"] = "0"

    # Hook tool execution to record accurate tool call and failure metrics
    total_tools_run = 0
    ...
    tools.run_tool = tracked_run_tool
    ...
    try:
        ...
        for turn in range(1, max_turns + 1):
            ...
            if not ans:
                ui._console.print(
                    "[yellow][Loop Engine] Turn yielded empty response. Retrying with state reminder...[/yellow]"
                )
                consecutive_failures += 1
                retry_prompt = "Continue executing the goal. Report current progress or next tool step."
                history.append({"role": "user", "content": retry_prompt})
                last_user_prompt = retry_prompt
                continue

            history.append({"role": "assistant", "content": ans})

            if turn_tool_failures > 0 or TOOL_ERROR_PATTERN.search(ans):
                consecutive_failures += 1
            else:
                consecutive_failures = 0

            if not no_log:
                _log_task_turn(workspace, turn, last_user_prompt, ans)

            # Check for completion marker
            if is_task_complete(ans):
                # Guard against hallucinating completion without running any tools
                if total_tools_run == 0 and any(
                    k in effective_goal.lower() for k in ("create", "write", "run", "test", "edit", "verify")
                ):
                    ui._console.print(
                        "[yellow][Loop Engine] Model declared completion without tool execution. Enforcing verification...[/yellow]"
                    )
                    prompt_enforce = (
                        "You declared TASK COMPLETE without executing tools. "
                        "You must use write_file to create the file and run_command to verify it. Execute the tools now."
                    )
                    history.append({"role": "user", "content": prompt_enforce})
                    last_user_prompt = prompt_enforce
                    continue
            ...
            next_step_prompt = "Continue with the next step. If completely finished and verified, output 'TASK COMPLETE'."
            history.append({"role": "user", "content": next_step_prompt})
            last_user_prompt = next_step_prompt
        ...
        # Restore monkey-patched tool runner and process-wide confirmation gates
        tools.run_tool = original_run_tool
        if prev_confirm_gates is not None:
            os.environ["AI_CONFIRM_GATES"] = prev_confirm_gates
        else:

...
def load_context_entries(
    context_file: str, stop_words: set[str] = STOP_WORDS
) -> list[dict[str, Any]]:
    ...
    try:
        ...
        with open(context_file, "r", encoding="utf-8") as f:
            lines = [
                ...
                if (c := l.strip()) and not c.startswith("#") and "--->" in c
            ]
        ...
        sys.stderr.write(
            f"\033[1;31m[sys] Error parsing context metadata: {e}\033[0m\n"
        )
        return []
...
def jaccard_search(
    ...
    for entry in entries:
        ...
        if score >= threshold:
            candidates.append(
                (score, entry["cmd"], entry.get("primary", entry["intent"]))
            )

...
CSI_U_REGEX = re.compile(
    r"(?:\x1b\[<|\x1b\[|\[<)?\d+;\d+;\d+[mM]|\x1b\[[0-9;]*[a-zA-Z~]|\x1b[\[\(\=][0-9;]*[a-zA-Z~]?"
)
ANSI_CLEAN_REGEX = re.compile(r"\x1b(?:[@-Z\\-_]|\[[0-?]*[ -/]*[@-~])")
REASONIX_STEP_RE = re.compile(
    r"^(?:\d+\.\s*|Step \d+:?\s*|Phase \d+:?\s*|\#{1,3}\s*)\*\*?([^\n\*:]+)\*\*?:?",
    re.IGNORECASE,
)
...
def format_model_name(name: str, max_len: int = 18) -> str:
    ...
    return (
        f".../{base}"
        if len(base) <= max_len
        else f"{base[: (max_len - 3) // 2]}...{base[-(max_len - 3) // 2 :]}"
    )
...
def copy_to_clipboard(text: str) -> bool:
    ...
    try:
        sys.stdout.write(
            f"\x1b]52;c;{base64.b64encode(text.encode('utf-8')).decode('utf-8')}\x07"
        )
        sys.stdout.flush()
    except OSError:
        pass
    tools = (
        [_CACHED_CLIPBOARD_TOOL]
...
def _format_tui_reasonix_text(text: str, theme: str = "code1") -> Text:
    ...
    for line in text.splitlines():
        ...
        if m := REASONIX_STEP_RE.match(clean_strip):
...
class ImageButton(Static):
    def on_click(self) -> None:
        if hasattr(self.app, "action_prompt_image_url"):
            self.app.action_prompt_image_url()


class CloseCardButton(Static):
    def on_click(self) -> None:
        if hasattr(self.app, "action_close_tips_card"):
            self.app.action_close_tips_card()
...
class Message(Static):
    ...
    def render(self) -> Any:
        ...
        if (
            ...
            and self._cached_theme == app_theme
            and self._cached_compact == compact_state
            and self._cached_borders == borders_on
        ):
        ...
        u_style = (
            "bold #888888"
            if app_theme in ("mono", "grok")
            else (
                "bold #89b4fa"
                if "code" in app_theme
                else ("bold #0265dc" if not is_dark else "bold cyan")
            )
        )
        ...
        bg_col = (
            ...
            if app_theme in ("mono", "grok")
                ...
                "#1a1a1a"
                if app_theme == "dark"
                else ("#1b1c2b" if is_dark else "#f0f0f4")
            )
        )
        ...
        if self.sender == "User":
            ...
            text = (
                raw_text.split("User Question:", 1)[-1].strip()
                if "User Question:" in raw_text
                else raw_text
            )
            if compact_state == 0:
                user_txt_col = (
                    "white"
                    if app_theme in ("mono", "grok", "dark")
                    else ("#303446" if not is_dark else "#c8d3f5")
                )
                res = Panel(
                    Text(text, style=user_txt_col),
                    box=LEFT_BAR,
            ...
            if "<think>" in text:
                ...
                items = []
                if before.strip():
                    items.append(Markdown(before.strip(), code_theme=code_fmt))
                ...
                if "</think>" in after:
                    ...
                    if show_think and think.strip():
                        items.append(
                            Panel(
                                _format_tui_reasonix_text(think.strip(), app_theme),
                                title="⚙",
                                title_align="left",
                                border_style=border_col,
                                box=ROUNDED,
                    ...
                    body = (
                        Group(*items)
                        if items
                        else Text("Thinking...", style="italic dim")
                    )
                else:
                    think = after.strip()
                    if show_think and think:
                        items.append(
                            Panel(
                                _format_tui_reasonix_text(think, app_theme),
                                title="⚙ Thinking...",
                                title_align="left",
                                border_style=border_col,
                                box=ROUNDED,
                ...
                body = (
                    Markdown(clean_text, code_theme=code_fmt)
                    if clean_text
                    else Text("...", style="italic dim")
                )
...
class AgentCommandProvider(Provider):
    async def search(self, query: str) -> Iterator[Hit]:
        ...
        cmds = [
            ...
            (
                "Copy Entire Chat Page",
                "copy_entire_chat",
                "Copy complete conversation transcript",
            ),
            ...
            (
                "Toggle Reasoning",
                "toggle_reasoning",
                "Enable or disable reasoning budget",
            ),
            (
                "Toggle Mode (Plan/Build)",
                "toggle_plan_build",
...
class LocalAITUI(App):
    ...
    def is_dark_theme(self) -> bool:
        return not any(
            kw in str(getattr(self, "theme", "code1")).lower()
            for kw in ["light", "latte", "day", "solarized-light", "dawn", "paper"]
        )
    ...
    def __init__(
        self, workspace_path: str, model_name: str, is_agent: bool | None = None
    ) -> None:
        super().__init__()
        ...
        agent_dir, cfg_file = (
            ...
            os.path.join(workspace_path, ".agent", "config.json"),
        )
        ...
        if is_agent is not None:
            ...
            ) and (os.path.exists(agent_dir) or "/projects/" in workspace_path)
        ...
        if not self.is_agent:
            ...
            is_yolo = (
                (env_gates == "0")
                if env_gates is not None
                else core.get_state("yolo_mode", False)
            )
            self.agent_mode, self.gates_enabled = (
                "Build" if is_yolo else "Plan",
                not is_yolo,
            )
            os.environ["AI_CONFIRM_GATES"] = "0" if is_yolo else "1"

        self.gate_auth_event = threading.Event()
        self.gate_auth_result = self.entering_gate_authorization = (
            self.entering_image_url
        ) = False
        ...
        self.base_skill = (
            skills_split[0] if skills_split else ("pi/pro" if self.is_agent else "chat")
        )
        self.on_demand_skill = skills_split[1] if len(skills_split) > 1 else None
        self.active_skill = (
            f"{self.base_skill} {self.on_demand_skill}".strip()
            if self.on_demand_skill
            else self.base_skill
        )

        self.memory_active, self.db_turns, self.tpm_count = (
            core.get_state("memory_active", False),
            0,
            0,
        )
        ...
        self.reasoning_active, self.reasoning_budget, self.entering_reasoning_budget = (
            core.get_state("reasoning_active", False),
            core.get_state("reasoning_budget", 500),
            False,
        )
        ...
        try:
            self.history: list[dict[str, Any]] = (
                json.loads(cli_hist) if cli_hist else []
            )
        except (json.JSONDecodeError, TypeError, ValueError):
            self.history = []

        self.generation_cancelled, self.active_response, self.stats_turns = (
            False,
            None,
            0,
        )
        self.borders_enabled = core.get_state("tui_borders_enabled", True)
        self.footer_hidden, self.sidebar_hidden, self.tips_card_hidden = (
            core.get_state("footer_hidden", True),
            core.get_state("sidebar_hidden", False),
            core.get_state("tips_card_hidden", False),
        )
    ...
    def notify(
        self, text: str, sys_prefix: bool = True, css_class: str = "sys-notice"
    ) -> None:
        self.chat_area.mount(
            Static(
                f"[dim white][sys] {text}[/dim white]" if sys_prefix else text,
                classes=css_class,
            )
        )
    ...
    def set_skill(self, skill_name: str) -> None:
        ...
        if hasattr(self, "lbl_skill"):
            self.lbl_skill.update(
                f"[dim]Skill[/dim]   [bold {fg} on {bg}] {skill_name} [/]"
            )

    def set_mode(self, mode_name: str) -> None:
        self.agent_mode = mode_name
        if hasattr(self, "lbl_mode"):
            self.lbl_mode.update(f"[dim]Mode[/dim]    {mode_name}")

    def set_reasoning(self, text: str) -> None:
        if hasattr(self, "lbl_reasoning"):
            self.lbl_reasoning.update(f"[dim]Reasoning[/dim] {text}")
    ...
    def action_prompt_image_url(self) -> None:
        if getattr(self, "entering_image_url", False):
            ...
            (
                ...
                self.pending_image_url,
                self.chat_input.placeholder,
            ) = True, "", "Enter Image URL (e.g. https://.../photo.png):"
    ...
    def ensure_system_context(self) -> None:
        if not any(m.get("role") == "system" for m in self.history):
            s_list = (
                [s.lstrip("-").lower() for s in self.active_skill.split() if s]
                if (
                    self.active_skill
                    and self.active_skill.lower() not in ("default", "none")
                )
                else []
            )
            s_content = (
                skills.load_skill_content(" ".join(s_list), SKILLS_DIR, CFG_DIR)
                if s_list
                else ""
            )
            ...
            if self.is_agent:
                sys_p = (
                    (s_content or BASE_PROMPT_AGENT)
                    ...
                    if use_map:
                        ...
                        for s_dir in search_dirs:
                            if os.path.exists(s_dir):
                                ...
                                if map_files:
                                    with open(
                                        os.path.join(s_dir, map_files[0]),
                                        "r",
                                        encoding="utf-8",
                                        errors="ignore",
                                    ) as mf:
    ...
    def get_db_status_string(self) -> str:
        return (
            ...
            if not self.is_agent
                ...
                f"active • {self.tpm_count} facts" if self.memory_active else "disabled"
            )
        )
    ...
    def update_welcome_banner(self) -> None:
        try:
            if self.query("#welcome-banner"):
                ...
                cmd_style = (
                    "bold #89b4fa"
                    if "code" in self.theme
                    else ("bold #0265dc" if not self.is_dark_theme else "bold cyan")
                )
                ...
                for k, a in [
                    ...
                    t.add_row(k, a)
                self.query_one("#welcome-banner", Static).update(
                    Panel(
                        t,
                        title=" ∿ PyTUI ",
                        title_align="left",
                        border_style=self.border_accent,
                        box=ROUNDED,
                        expand=False,
                    )
                )
        ...
        with Horizontal(id="layout"):
            with Vertical(id="main-container"):
                ...
                with Horizontal(id="input-pane"):
                    yield Static("▌\n▌\n▌", id="input-bar")
                    yield Input(
                        placeholder="Ask your agent anything...", id="chat-input"
                    )
            ...
            with Vertical(id="sidebar"):
                with Vertical(classes="sidebar-section"):
                    ...
                    yield Static(
                        f"[dim]Model[/dim]   {format_model_name(self.model_name)}",
                        id="lbl-model",
                        classes="sidebar-val",
                    )
                    yield Static(
                        f"[dim]Dir[/dim]     {format_dir_path(self.workspace_path)}",
                        id="lbl-dir",
                        classes="sidebar-val",
                    )
                    yield Static(
                        f"[dim]Skill[/dim]   {self.active_skill}",
                        id="lbl-skill",
                        classes="sidebar-val",
                    )
                    yield Static(
                        f"[dim]Mode[/dim]    {self.agent_mode}",
                        id="lbl-mode",
                        classes="sidebar-val",
                    )
                    yield Static(
                        "[dim]Harness[/dim] Chat Mode",
                        id="lbl-harness",
                        classes="sidebar-val",
                    )
                    yield Static(
                        "[dim]Image[/dim]   None", id="lbl-image", classes="sidebar-val"
                    )

                with Vertical(classes="sidebar-section"):
                    yield Static("SETTINGS", classes="sidebar-label")
                    yield Static(
                        "[dim]Reasoning[/dim] Disabled",
                        id="lbl-reasoning",
                        classes="sidebar-val",
                    )
                    yield Static(
                        "[dim]Voice[/dim]   Disabled",
                        id="lbl-voice",
                        classes="sidebar-val",
                    )
                    yield Static(
                        "[dim]TTS[/dim]     Disabled",
                        id="lbl-tts",
                        classes="sidebar-val",
                    )
    ...
    def on_mount(self) -> None:
        ...
        try:
            self.theme = (
                "mono"
                if core.get_state("tui_theme", "code1") == "grok"
                else core.get_state("tui_theme", "code1")
            )
        ...
        self.lbl_skill, self.lbl_mode = (
            self.query_one("#lbl-skill", Static),
            self.query_one("#lbl-mode", Static),
        )
        self.lbl_harness, self.lbl_reasoning = (
            self.query_one("#lbl-harness", Static),
            self.query_one("#lbl-reasoning", Static),
        )
        self.lbl_database, self.lbl_stats = (
            self.query_one("#lbl-database", Static),
            self.query_one("#lbl-stats", Static),
        )
        self.lbl_voice, self.lbl_tts, self.lbl_image = (
            self.query_one("#lbl-voice", Static),
            self.query_one("#lbl-tts", Static),
            self.query_one("#lbl-image", Static),
        )

        is_py_profile = "py-" in self.active_skill.lower()
        use_ipython = (
            (is_py_profile or (ipython and ipython.is_ipython_enabled()))
            if self.is_agent
            else False
        )
        if hasattr(self, "lbl_harness"):
            self.lbl_harness.update(
                "[dim]Harness[/dim] "
                + (
                    "NOOA IPython"
                    if use_ipython
                    else ("Native Tools" if self.is_agent else "Chat Mode")
                )
            )
        ...
        if hasattr(voice, "is_bridge_running"):
            ...
            self.lbl_voice.update(
                f"[dim]Voice[/dim]   {'Active (auto)' if (is_act and auto_st) else ('Active (edit)' if is_act else 'Disabled')}"
            )

        if hasattr(tts, "is_tts_enabled"):
            self.lbl_tts.update(
                f"[dim]TTS[/dim]     {'Active' if tts.is_tts_enabled() else 'Disabled'}"
            )

        os.environ["AI_SHOW_THINKING"] = (
            "1" if core.get_state("show_thinking", True) else "0"
        )
        self.set_skill(self.active_skill)
        ...
        self.set_reasoning(
            f"{self.reasoning_budget} tokens" if self.reasoning_active else "Disabled"
        )
        self.update_welcome_banner()
        ...
        if len(self.history) > 1:
            ...
            for msg in self.history:
                ...
                if r == "user" and c:
                    self.chat_area.mount(Message("User", c))
                elif r == "assistant" and c:
                    self.chat_area.mount(Message("Agent", c))
    ...
    def action_toggle_plan_build(self) -> None:
        if not self.is_agent:
            return
        self.agent_mode, self.gates_enabled = (
            ("Build", False) if self.agent_mode == "Plan" else ("Plan", True)
        )
    ...
    def update_stats_ui(self, turns: int, tps: float, elapsed: float) -> None:
        if hasattr(self, "lbl_stats"):
            self.lbl_stats.update(
                f"[dim]Turns[/dim]     {turns} @ {f'{tps:.1f} t/s' if tps > 0 else '-- t/s'}"
            )
    ...
    def action_copy_last_response(self) -> None:
        if last := next(
            ...
            copy_to_clipboard(
                last.split("</think>", 1)[-1].strip() if "</think>" in last else last
            )
            self.notify("Copied latest agent response to clipboard.")
        else:
            self.notify("No response available to copy yet.")

    def action_copy_entire_chat(self) -> None:
        if transcript := [
            f"❯ USER: {m['content']}"
            if m.get("role") == "user"
            else f"AGENT:\n{THINK_TAGS_RE.sub('', m['content']).strip()}"
        ...
        if os.path.isfile(full_p):
            try:
                ...
                self.history.append(
                    {
                        "role": "user",
                        "content": f"[FILE LOADED: {file_path}]\n```\n{content}\n```",
                    }
                )
                self.notify(
                    f"Loaded file content into active context: [bold]{file_path}[/bold]"
                )
            except (OSError, UnicodeDecodeError) as e:
                self.notify(
                    f"[bold red]Error reading file: {e}[/bold red]", sys_prefix=False
                )
        else:
            self.notify(
                f"[bold red]File not found: {file_path}[/bold red]", sys_prefix=False
            )
    ...
    async def handle_task_command(self, task_args: str = "") -> None:
        ...
        assistant_msg = Message(
            "Agent", f"[task] Executing Goal loop: [italic]{goal}[/italic]..."
        )
        ...
        def _run_task_sub():
            ...
            if os.path.exists(ralph_bin):
                try:
                    ...
                    res = subprocess.run(
                        [sys.executable, ralph_bin, task_args],
                    ...
                    for line in plain_text.splitlines():
                        ...
                        if not l_strip or (
                            (
                                "Working..." in l_strip
                                or any(c in l_strip for c in "⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏")
                            )
                            and "working •" not in l_strip
                        ):
                            continue
                    ...
                    for line in clean_lines:
                        if line.startswith("[loop] Starting autonomous execution"):
                            m = re.search(
                                r"in\s+(\S+)\s+\(max\s+(\d+)\s+turns\)\s+::\s+Goal:\s*(.*)",
                                line,
                            )
                            if m:
                    ...
                    self.history.append(
                        {"role": "assistant", "content": clean_out or "Task complete."}
                    )
                    self.refresh_db_counts()
                except (OSError, subprocess.SubprocessError, TimeoutError) as e:
                    self.call_from_thread(
                        assistant_msg.update_content,
                        f"[red][sys] Task loop error: {e}[/red]",
                    )
            else:
                self.call_from_thread(
                    assistant_msg.update_content,
                    "[red][sys] tools/loop/ralph.py script not found.[/red]",
                )
    ...
    async def handle_slash_command(self, cmd: str) -> None:
        ...
        if root in ("/help", "/h"):
            ...
            for c, d in [
                ...
                ("/copy", "Copy page"),
                ("/m", "Memory"),
                ("/clear, /c", "Soft clear chat"),
                ("/reset", "Hard workspace reset"),
                ("/tok", "Tokens"),
                ("/sync", "Sync index"),
                ("/s <q>", "Skill"),
                ("/t <toks>", "Reasoning"),
                ...
                t.add_row(c, d)
            border_col = (
                "bright_white" if self.theme in ("mono", "grok") else self.border_accent
            )
            ...
            if args:
                ...
                if t_arg in self.THEMES:
                    self.theme = t_arg
                    self.notify(
                        f"Theme switched to [bold]{self.theme}[/bold].",
                        css_class="theme-notice",
                    )
                else:
                    self.notify(
                        f"Unknown theme '{t_arg}'. Options: {', '.join(self.THEMES)}"
                    )
            else:
            ...
            if args:
                if not ipython.is_ipython_enabled():
                    ...
                    if hasattr(self, "lbl_harness"):
                        self.lbl_harness.update("[dim]Harness[/dim] NOOA IPython")
                    self.notify("NOOA IPython harness enabled.")
                    if "py-" not in self.active_skill:
                        self.notify(
                            f"[yellow]Warning: Active skill '{self.active_skill}' is a classic JSON skill. Use a Py profile (e.g. pi/py-pro) for best in-kernel SDK results.[/yellow]",
                            sys_prefix=False,
                        )
                ...
                if hasattr(self, "lbl_harness"):
                    self.lbl_harness.update(
                        "[dim]Harness[/dim] "
                        + ("NOOA IPython" if active else "Native Tools")
                    )
                self.notify(
                    f"NOOA IPython harness {'enabled (single tool mode)' if active else 'disabled (classic tools)'}."
                )
            ...
            val_str = (
                f"Active ({'auto' if auto_mode else 'edit'})" if active else "Disabled"
            )
            if hasattr(self, "lbl_voice"):
                self.lbl_voice.update(f"[dim]Voice[/dim]   {val_str}")
            self.notify(
                f"Voice to text {'active (' + mode_str + ' mode, port 9999)' if active else 'disabled'}."
            )
            ...
            if hasattr(self, "lbl_tts"):
                self.lbl_tts.update(
                    f"[dim]TTS[/dim]     {'Active' if active else 'Disabled'}"
                )
            ...
            if not self.is_agent:
                self.notify(
                    "Plan/Build modes are only available in project workspace sessions.",
                    sys_prefix=False,
                )
            else:
                if (
                    (root == "/plan" and self.agent_mode != "Plan")
                    or (root == "/build" and self.agent_mode != "Build")
                    or root in ("/g", "/yolo")
                ):
                    self.action_toggle_plan_build()
            ...
            panel = Panel(
                Group(
                    Text.assemble(
                        ("Context Window: ", "dim"),
                        (f"{total_toks:,}", f"bold {status_col}"),
                        (f"/{limit:,} tokens ", "dim"),
                        (f"({pct:.1f}%)", f"bold {status_col}"),
                    ),
                ...
                if sub_cmd in ("off", "clear", "reset", "none", "remove"):
                    ...
                    if self.history and self.history[0].get("role") == "system":
                        ...
                        if "### Loaded On-Demand Skill:" in sys_c:
                            self.history[0]["content"] = sys_c.split(
                                "### Loaded On-Demand Skill:"
                            )[0].strip()
                    os.environ["AI_ACTIVE_SKILL"] = self.base_skill
                    self.notify(
                        f"On-demand skill removed. Reverted to base skill: [bold]{self.base_skill}[/bold]"
                    )
                elif content := skills.load_skill_content(args, SKILLS_DIR, CFG_DIR):
                    s_name, s_text = (
                        content if isinstance(content, tuple) else (args, content)
                    )
                    ...
                    if self.history and self.history[0].get("role") == "system":
                        ...
                        self.history.insert(
                            ...
                            {
                                "role": "system",
                                "content": f"### Loaded On-Demand Skill: {s_name}\n{s_text}\n",
                            },
                        )
                    os.environ["AI_ACTIVE_SKILL"] = combined_skill
                    self.notify(
                        f"Active skills: [bold]{combined_skill}[/bold] (Swapped on-demand skill to [bold]{s_name}[/bold])"
                    )
                else:
                    self.notify(
                        f"No skill blueprint file found for '[bold]{args}[/bold]'."
                    )
            else:
                self.notify("Usage: /skill <query>, /s <query>, or /s off")
                ...
                if sub in ("hide", "off", "mute", "quiet"):
                    ...
                    for child in self.chat_area.children:
                        if isinstance(child, Message):
                            child.refresh(layout=True)
                    self.notify(
                        "Thinking display hidden (thinking mode remains active)."
                    )
                    ...
                    status = (
                        f"{self.reasoning_budget} tokens"
                        if self.reasoning_active
                        else "Disabled"
                    )
                    self.set_reasoning(status)
                    self.notify(
                        f"Deep reasoning {'enabled' if self.reasoning_active else 'disabled'} ({status})."
                    )
                else:
            ...
            if hasattr(self, "handle_meta_chat_command"):
                ...
                self.notify(
                    f"Preset command '{root}' is not configured for this workspace profile."
                )
        else:
            self.notify(
                f"Unknown command '{root}'. Type [bold]/help[/bold] for commands."
            )
    ...
    def prompt_tui_confirm(self, prompt_text: str) -> bool:
        ...
        def _show():
            self.entering_gate_authorization, self.current_gate_prompt = (
                True,
                prompt_text,
            )
    ...
    def process_query_worker(self, query: Any) -> None:
        ...
        for notice in self.chat_area.query(".sys-notice, .theme-notice"):
            try:
                self.call_from_thread(notice.remove)
            except Exception:
                pass
        ...
        try:
            ...
            if self.is_agent and self.memory_active and isinstance(query, str):
                try:
                    tpm_ctx = core.run_mod(
                        "ai-agent-memories", "tpm-get", self.safe_name
                    )
                except (OSError, subprocess.SubprocessError):
                    pass
            ...
            try:
                sys_ctx = (
                    skills.get_system_context(
                        user_text, CONTEXT_FILE, STOP_WORDS, SKILLS_DIR, CFG_DIR
                    )
                    if (
                        isinstance(query, str) and hasattr(skills, "get_system_context")
                    )
                    else ""
                )
            ...
            accumulated, start_time, first_token_time, token_count = (
                "",
                time.perf_counter(),
                None,
                0,
            ...
            for _round in range(10 if self.is_agent else 1):
                ...
                configs = (
                    agent_cloud.get_active_configs(self.history) if agent_cloud else []
                )
                ...
                if not response:
                    raise Exception(
                        "Failed to establish streaming response connection to AI engine or cloud backups."
                    )
                    ...
                    for line in response:
                        ...
                        try:
                            ...
                            if text_chunk and "Final Answer:" in text_chunk:
                                text_chunk = FINAL_ANSWER_RE.sub(
                                    "", text_chunk
                                ).lstrip()
                            ...
                            if (text_chunk or thinking_chunk) and (
                                now - last_ui_update >= 0.08
                            ):
                                last_ui_update = now
                                self.call_from_thread(
                                    assistant_msg.update_content, accumulated
                                )
                                self.call_from_thread(
                                    self.chat_area.scroll_end, animate=False
                                )
                ...
                calls = (
                    [v for _, v in sorted(tool_calls_map.items())]
                    if tool_calls_map
                    else None
                )
                ...
                for tc in calls:
                    ...
                    args = (
                        core._heal_tool_args(raw_args)
                        if hasattr(core, "_heal_tool_args")
                    ...
                    if user_aborted:
                        ...
                        if self.gates_enabled and not self.prompt_tui_confirm(
                            f"{fname} {brief}"
                        ):
                            result, user_aborted = (
                                f"[denied] user rejected {fname}",
                                True,
                            )
                        else:
                            self.call_from_thread(
                                self.notify,
                                f"∗ {verb} • [bold cyan]{fname}[/bold cyan] [italic]{brief}[/italic]",
                            )
                            old_g = os.environ.get("AI_CONFIRM_GATES")
                            try:
                                os.environ["AI_CONFIRM_GATES"] = "0"
                                result = core._run_edit_tool(
                                    fname, args, self.workspace_path
                                )
                            except (
                                OSError,
            ...
            gen_dur = (
                max(0.001, end_time - first_token_time)
                if first_token_time
                else total_elapsed
            )
            ...
            tps = (
                (out_toks / gen_dur)
                if first_token_time and out_toks > 0
                else out_toks / total_elapsed
            )

            self.stats_turns += 1
            self.call_from_thread(
                self.update_stats_ui, self.stats_turns, tps, total_elapsed
            )
            ...
            if self.generation_cancelled:
                self.call_from_thread(
                    assistant_msg.update_content, (accumulated or "") + " (stopped)"
                )
            else:
                self.call_from_thread(assistant_msg.update_content, f"Error: {e}")
        finally:
    ...
    async def on_input_submitted(self, event: Input.Submitted) -> None:
        ...
        if getattr(self, "entering_image_url", False):
            if not getattr(self, "pending_image_url", ""):
                ...
                img_url, prompt_text = (
                    self.pending_image_url,
                    query or "Describe this image in detail.",
                )
                (
                    self.entering_image_url,
                    self.pending_image_url,
                ...
                self.run_worker(
                    lambda: self.process_query_worker(multimodal_payload), thread=True
                )
                return
        ...
        if getattr(self, "entering_gate_authorization", False):
            ...
            self.gate_auth_event.set()
            self.notify(
                f"[dim]Gate: {'Authorized' if is_yes else 'Denied'}[/dim]",
                sys_prefix=False,
            )
            return

        if self.entering_reasoning_budget:
            self.entering_reasoning_budget, self.chat_input.placeholder = (
                False,
            ...
            if not query:
                ...
                try:
                    ...
                    self.notify(
                        "[dim]Invalid budget. Deep reasoning disabled.[/dim]",
                        sys_prefix=False,
                    )
            return
        ...
        if self.pending_skill_prefix:
            query, self.pending_skill_prefix = (
                f"{self.pending_skill_prefix} {query}",
                None,
            )
            self.chat_input.placeholder = "Ask your agent anything..."
        ...
        if query.lower().startswith("file "):
            ...
            if len(parts) > 1:
                await self.handle_view_file(parts[1].strip())
            return
    ...
    def action_stop_generation(self) -> None:
        if self.chat_input.disabled or getattr(
            self, "entering_gate_authorization", False
        ):
    ...
    def update_footer_visibility(self) -> None:
        try:
            ...
            self.query_one("#input-toggle", FooterToggle).update(
                "▲ Show" if self.footer_hidden else "▼ Hide"
            )
        except (KeyError, AttributeError):
            pass
    ...
    def action_cycle_theme(self) -> None:
        try:
            ...
            self.notify(
                f"Theme: {self.theme}", sys_prefix=False, css_class="theme-notice"
            )
    ...
    def action_toggle_borders(self) -> None:
        ...
        if hasattr(self, "chat_area"):
            ...
            self.chat_area.refresh(layout=True)
        self.notify(
            f"Message borders: {'Enabled' if self.borders_enabled else 'Disabled (Borderless)'}",
            sys_prefix=False,
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        model = (
            configs[0][2].get("model", "local-model")
            if configs
            else ui.get_local_model_name()
        )
    except (ImportError, AttributeError, KeyError, IndexError, OSError):
        model = ui.get_local_model_name()

    try:
        LocalAITUI(workspace, model).run()
    finally:
        try:

    ...
    try:
        ...
        with urlreq.urlopen(req, timeout=0.5) as r:
            return os.path.basename(
                json.loads(r.read().decode("utf-8"))["data"][0]["id"]
            )
    except Exception:
        return "local-model"
...
def draw_session_box(
    ...
    mem_status = (
        f"active ({tpm_count} facts, {db_turns} turns)" if memory_active else "disabled"
    )
    ...
    if box_style == 5:
        ...
        title_text = (
            f" {base_title} [sub-agent #{sub_id}] " if sub_id else f" {base_title} "
        )
        panel = Panel(
            table,
    ...
    _console.print(
        f"[dim][sys] Startup context: {len(active_system_prompt) // 4:,} tokens[/dim]\n"
    )
    try:
        sys.stderr.write("\033[?25h")
        sys.stderr.flush()
...
def run_interactive_selection(
    ...
    if RE_UNSAFE_SHELL_CHARS.search(intent) or not (
...
def show_help() -> None:
    ...
    _console.print(
        ...
        Panel(
            Group(
                header, Text(""), Text("  Commands:", style="bold yellow"), cmd_table
            ),
            title=" Help & Commands ",
            title_align="left",
    ...
    try:
        while True:
            ...
            if char in ("\t", "y", "Y"):
                ...
                if not is_yolo:
                    sys.stderr.write(
                        "\033[1;36mEnable Autonomous YOLO mode? [y/N]: \033[0m"
                    )
                    sys.stderr.flush()
                    c = get_key().lower()
                    sys.stderr.write("y\n" if c == "y" else "n\n")
                ...
                current_idx = (
                    current_idx + (1 if char == "\x1b[B" else -1) + num_opts
                ) % num_opts
    finally:
        sys.stderr.write("\033[?25h")
        sys.stderr.flush()

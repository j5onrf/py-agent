...
BLACKLIST = frozenset({"files", "file", "file_list", "project", "code", "description", "features", "dependencies", "project_type", "directory", "folder", "workspace"})
...
def connect_db(db_path: str) -> sqlite3.Connection:
    ...
    try:
        ...
        conn.execute("CREATE TABLE IF NOT EXISTS tpm_memories (key TEXT PRIMARY KEY, value TEXT, timestamp INTEGER);")
        conn.commit()
    ...
    if os.path.exists(db_path):
        try:
            with closing(connect_db(db_path)) as conn:
                if rows := conn.cursor().execute("SELECT key, value FROM tpm_memories ORDER BY key").fetchall():
                    return "### TEMPORAL PERSONALITY MEMORY (TPM):\nThe following are compiled, up-to-date facts and style preferences about the user:\n" + "\n".join(f"* **{k}**: {v}" for k, v in rows) + "\n"
        except sqlite3.Error: pass
    return ""


def tpm_reconcile(workspace: str, facts: Optional[dict[str, str]] = None) -> None:
    """Direct in-memory or stdin reconciliation into SQLite (< 0.2ms)."""
    if facts is None:
        try: facts = json.loads(sys.stdin.read().strip())
        except Exception: return
    ...
    with closing(connect_db(os.path.join(SESSIONS_DIR, f"{workspace}.db"))) as conn:
        with conn:
            ...
            for k, v in facts.items():
                ...
                if k_clean in BLACKLIST: continue
...
def tpm_clear(workspace: str) -> None:
    ...
    if os.path.exists(db_path):
        try:
            with closing(connect_db(db_path)) as conn:
                with conn: conn.cursor().execute("DELETE FROM tpm_memories")
        except sqlite3.Error: pass
...
def get_tpm_count(workspace: str) -> int:
    ...
    if os.path.exists(db_path):
        try:
            with closing(connect_db(db_path)) as conn:
                return conn.cursor().execute("SELECT COUNT(*) FROM tpm_memories").fetchone()[0]
        except sqlite3.Error: pass
    return 0
...
def search_past_context(workspace: str, query: str) -> str:
    ...
    with closing(connect_db(db_path)) as conn:
        try:
            rows = conn.cursor().execute("SELECT id, tokens, timestamp FROM turns WHERE workspace = ? ORDER BY timestamp DESC LIMIT 500", (workspace,)).fetchall()
        except sqlite3.OperationalError: return ""

        matching_ids = []
        for row_id, tokens, ts in rows:
            t_tokens = set(tokens.split()) if tokens else set()
            if (score := len(q_tokens & t_tokens) / len(q_tokens | t_tokens) if (q_tokens & t_tokens) else 0.0) >= 0.35:
        ...
        for score, row_id, ts in top_matches:
            try:
                row = conn.cursor().execute("SELECT user_msg, assistant_msg FROM turns WHERE id = ?", (row_id,)).fetchone()
                if row: candidates.append((score, row[0], row[1], ts))
    ...
    if len(args := sys.argv[1:]) >= 2:
        ...
        if cmd == "get-context":
            if out := search_past_context(ws, args[2] if len(args) > 2 else ""):
                if out not in ("__CANCELLED__", "__DISABLE_MEMORY__"): print(out)
        elif cmd == "tpm-reconcile": tpm_reconcile(ws)
        elif cmd == "tpm-get":
            if res := tpm_get(ws): print(res)
        elif cmd == "tpm-clear": tpm_clear(ws)
        elif cmd == "get-tpm-count": print(get_tpm_count(ws))

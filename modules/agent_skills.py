...
PAGER_STRIP_RE: re.Pattern = re.compile(
    r"\|\s*(leaf|mdcat|cat|glow|view)\b.*$", re.IGNORECASE
)
...
def ensure_mysys_exists(skills_dir: str, cfg_dir: str) -> None:
    if not os.path.exists(os.path.join(skills_dir, "system", "mysys.md")):
        try:
            subprocess.run(
                [sys.executable, os.path.join(cfg_dir, "tools", "generate-profile")],
                check=False,
            )
        except Exception:
            pass
    ...
    for skill in [s.lstrip("-").lower() for s in skills_str.split()]:
        if sf := find_skill_file(skills_dir, skill):
            ...
            try:
                ...
                sys.stderr.write(
                    f"\033[1;31mError loading skill '{skill}': {e}\033[0m\n"
                )
    return "\n\n".join(contents)
...
def _exec_tool_cmd(cmd: str, interactive: bool = False) -> str:
    try:
        ...
        out = subprocess.check_output(
            sanitized, shell=True, text=True, timeout=15, cwd=workspace, env=env
        ).strip()
        return f"{out}\n" if out else "Action executed successfully.\n"
    except subprocess.CalledProcessError:
        sys.stderr.write(
            "\033[1;31m[sys] Tool execution failed or was cancelled.\033[0m\n"
        )
        sys.stderr.flush()
        return "__ABORT_TURN__"
...
def get_system_context(
    query: str, context_file: str, stop_words: set[str], skills_dir: str, cfg_dir: str
) -> str:
    ...
    for entry in context.load_context_entries(context_file, stop_words):
        ...
        if any(
            ...
            args = " ".join(
                ...
                or (
                    context.tokenize(w, stop_words)
                    and context.tokenize(w, stop_words)[0] not in intent_tokens
                )
            )
...
def load_skill_blueprints(
    base_skills_dir: str, stop_words: set[str]
) -> list[dict[str, Any]]:
            ...
            for f in files:
                if f.lower().endswith(".md"):
                    ...
                    try:
                        ...
                        lines = [
                            l.strip()
                            for l in (body or content).splitlines()
                            if l.strip()
                        ]
                        ...
                        # 2. Legacy Header: # [SKILL] name ---> intent1, intent2
                            ...
                            lines
                            and lines[0].startswith("# [SKILL]")
                            and "--->" in lines[0]
                        ):
                            header, intents_raw = lines[0].split("--->", 1)
                            skill_name = (
                                header.replace("# [SKILL]", "").replace("#", "").strip()
                            )
                            intents = [
                                i.strip().lower()
                                for i in intents_raw.split(",")
                                if i.strip()
                            ]
                            desc = next(
                                (
                                    ...
                                    and not RE_METADATA_LINE.match(l)
                                ),
                                "",
                            )
                        ...
                        # 3. Standard Markdown Title / Filename fallback
                            ...
                            skill_name = next(
                                ...
                                base_name.replace("-", " ").replace("_", " ").title(),
                            )
                            intents = list(
                                set(
                                    RE_SKILL_SPLIT.split(base_name.lower())
                                    + context.tokenize(skill_name, stop_words)
                                )
                            )
                            desc = next(
                        ...
                        blueprints.append(
                            {
                                ...
                                "desc": clean_desc,
                                "intents": intents,
                                "tokens": context.tokenize(
                                    " ".join(intents), stop_words
                                ),
                            }
                        )
                    except Exception:
...
def run_skill_selector(
    ...
    try:
        while True:
            q_tokens = (
                set(context.tokenize(search_query, stop_words))
                if search_query
                else set()
            )
            ...
            for s in skill_list:
                if not search_query:
                    ...
                    score = (
                        len(q_tokens & s_tokens) / len(q_tokens | s_tokens)
                        if (q_tokens & s_tokens)
                        else 0.0
                    )
                    if (
                        sq_lower in s["name"]
                        or sq_lower in os.path.basename(s["path"]).lower()
                        or any(sq_lower in i for i in s["intents"])
                    ):
                        score = max(score, 0.8)
                    if score > 0.0:
            ...
            if num_opts == 0:
                sys.stderr.write(
                    f'\r\x1b[2K\033[1;30m[00/00]\033[0m ❯ \x1b[1;31m[No matches]\x1b[0m for: \033[1;33m{search_query}\033[0m\n\r\x1b[2K\033[3m   "Backspace to delete"\033[0m [Esc to exit]: '
                )
            else:
                ...
                desc = (
                    ...
                    else clean_desc[: max_desc_len - 3] + "..."
                )
                filter_ind = (
                    f" \033[90m| Filter: \033[1;33m{search_query}\033[0m"
                    if search_query
                    else ""
                )
                sys.stderr.write(
                    f'\r\x1b[2K\033[1;30m[\033[1;32m{current_idx + 1:02d}/{num_opts:02d}\033[1;30m]\033[0m ❯ \x1b[1;36m[skill]\x1b[0m \033[1;32m{sel["name"]}\033[0m \033[90m({sel["rel_path"]}){filter_ind}\033[0m\n\r\x1b[2K\033[3m   "{desc}"\033[0m [↵ load  Type to filter  Esc]: '
                )
            sys.stderr.flush()
            ...
            if key in ("\x03", "\x1b"):
                ...
                if num_opts > 0:
                    ...
                    try:
                        ...
                        cat = (
                            "personality"
                            if "personality" in sel["path"]
                            else ("code" if "code" in sel["path"] else "system")
                        )
                        ...
                        for s_n, s_b in raw_blocks:
                            s_cat = (
                                ...
                                if any(
                                    p in s_n
                                    for p in ("caveman", "pirate", "personality")
                                )
                                else "other"
                            )
                        ...
                        new_blocks = "\n\n".join(
                            f"### Loaded On-Demand Skill: {n}\n{b}"
                            for n, b in active_skills
                        )
                        chat_history[0]["content"] = f"{base_p}\n\n{new_blocks}\n"

                        s_name = sel["name"].replace(" ", "-")
                        sys.stderr.write(
                            f"{clear_2_lines}\033[1;32m✓ Skill '{sel['name']}' successfully loaded.\033[0m\n\n"
                        )
                        return chat_history, s_name
                    except Exception as e:
                        sys.stderr.write(
                            f"{clear_2_lines}\033[1;31m[sys] Failed to load skill: {e}\033[0m\n"
                        )
                else:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        sys.argv.extend(["", ""])
    hist, name = run_skill_selector(
        sys.argv[1], sys.argv[2], os.path.join(CFG_DIR, "skills"), stop_words
    )
    if hist:
